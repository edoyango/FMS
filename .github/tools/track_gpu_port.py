...
The same override is available as a one-line marker that needs no closing tag
and applies to just the next line:

    !@noport                exclude the next code statement (here a whole-array
    a(:,:) = b(:,:)         copy) from "portable".

    !@toport                force the next code statement (here a scalar
    energy = energy + dV    reduction) into "portable".

It applies to exactly one logical line: continuation (`&`) lines are joined,
and every `;`-separated statement on the line is included (so a marker above a
`do j=..; do i=..` line covers both loop headers). On a plain `do` header it is
header-only — it removes just that `do` line, not the loop body — so an outer
driver loop can be flagged while its inner loops stay portable. The next line
must be a real code statement: a blank line, comment, `!$omp` directive, another
marker, or end-of-file is a structural error (put any explanation as a trailing
comment on the marker line). A one-line marker may not appear inside an open
`!@start`/`!@end` region — the two forms coexist in a file but never nest.
...
`--exclude PREFIX` (repeatable) drops any file whose --src-root-relative path
falls under PREFIX from both the structural scan and the coverage report —
e.g. `--exclude test_fms` to track porting progress in the library only, not
in test infrastructure that happens to use `do concurrent`/`omp target` itself.
...
# Single-line variants: no closing tag; each tags just the next logical line.
NOPORT_LINE_RE = re.compile(r'^\s*!\s*@noport\s*(?:!.*)?$', re.IGNORECASE)
TOPORT_LINE_RE = re.compile(r'^\s*!\s*@toport\s*(?:!.*)?$', re.IGNORECASE)
...
def parse_ported_regions(path):
    ...
    noport_regions = []
    toport_regions = []
    # Same, for the one-line "!@noport"/"!@toport" markers: (marker_line, target_start,
    # target_span), kept only for the post-pass redundancy/contradiction checks.
    noport_line_markers = []
    toport_line_markers = []
    ...
    open_marker = None
    # The next code line a one-line "!@noport"/"!@toport" applies to, or None:
    # (kind, marker_line), awaiting the immediately-following statement.
    pending_line_marker = None
    ...
    for start, end, text_joined in iter_logical_lines(lines):
        ...
        if pending_line_marker is not None:
            kind, mstart = pending_line_marker
            if not stripped or stripped.startswith('!'):
                raise StructuralError(
                    f'{path}:{mstart}: "!@{kind}" must be immediately followed by a code '
                    f'statement, not a blank line, comment, "!$omp" directive, or another '
                    f'marker (put any explanation as a trailing comment on the marker line)')
            span = set(range(start, end + 1))
            if kind == 'noport':
                fr.manual_noport.update(span)
                noport_line_markers.append((mstart, start, span))
            else:
                fr.manual_toport.update(span)
                toport_line_markers.append((mstart, start, span))
            pending_line_marker = None
            # Fall through (no continue): the target line still needs normal
            # classification — a "do" target must still open on do_stack, an array
            # assignment must still populate fr.array_syntax, etc. The marker only
            # *adds* these lines to manual_noport/manual_toport; the `portable`
            # property then applies them over the structural result.
        if not stripped or stripped.startswith('!') and not OMP_RE.match(stripped):
            if stripped.startswith('!') and not OMP_RE.match(stripped):
                ...
                if NOPORT_START_RE.match(stripped):
                    ...
                    open_marker = None
                elif NOPORT_LINE_RE.match(stripped):
                    if open_marker is not None:
                        raise StructuralError(
                            f'{path}:{start}: "!@noport" inside an open "!@start '
                            f'{open_marker[0]}" region opened at line {open_marker[1]} '
                            f'(single-line and region markers cannot nest)')
                    pending_line_marker = ('noport', start)
                elif TOPORT_LINE_RE.match(stripped):
                    if open_marker is not None:
                        raise StructuralError(
                            f'{path}:{start}: "!@toport" inside an open "!@start '
                            f'{open_marker[0]}" region opened at line {open_marker[1]} '
                            f'(single-line and region markers cannot nest)')
                    pending_line_marker = ('toport', start)
            continue
    ...
    if pending_line_marker is not None:
        raise StructuralError(
            f'{path}: one-line "!@{pending_line_marker[0]}" at line '
            f'{pending_line_marker[1]} has no following code statement (end of file)')
    if do_stack:
        fr.warnings.append(
            f'{path}: {len(do_stack)} unbalanced "do" opener(s) at EOF (parser limitation)')

    # Flag markers that fight the structural heuristic: a "noport" overlapping
    # already-ported code (contradiction), or a "toport" that is redundant because the
    # region/line is already structurally portable. Region and one-line markers are
    # checked the same way, over the full marked span.
    ...
    for mstart, tstart, span in noport_line_markers:
        if span & fr.ported:
            fr.warnings.append(
                f'{path}:{mstart}: one-line "!@noport" tags automatically-detected '
                f'ported code at line {tstart} (contradiction)')
        elif not (span & auto_portable):
            fr.warnings.append(
                f'{path}:{mstart}: one-line "!@noport" tags line {tstart}, which is '
                f'already not portable (redundant)')
    for mstart, tstart, span in toport_line_markers:
        if span.issubset(auto_portable):
            fr.warnings.append(
                f'{path}:{mstart}: one-line "!@toport" tags line {tstart}, which is '
                f'already structurally portable (redundant)')
...
def is_excluded(rel_path, excludes):
    """True if rel_path (relative to --src-root) falls under any --exclude prefix."""
...
def main():
    ...
    ap.add_argument('--src-root', required=True, help='Root of the source tree to scan for ported regions')
    ap.add_argument('--gcov-dir', help='Directory containing .gcov files (recursively searched)')
    ap.add_argument('--exclude', action='append', default=[],
                     help='Src-root-relative path prefix to exclude from scanning and reporting '
                          '(e.g. test_fms) — repeatable')
    ...
    if args.exclude:
        src_files = [f for f in src_files if not is_excluded(f.relative_to(src_root), args.exclude)]
    for f in src_files:
        if not f.is_file():
    ...
    for gf in gcov_files:
        ...
        if args.exclude:
            try:
                ...
                rel = None
            if rel is not None and is_excluded(rel, args.exclude):
                continue
